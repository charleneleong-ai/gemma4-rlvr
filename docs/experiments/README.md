...
- [`v2_slot_grounded.md`](dd_explainer/two_stage/v2_slot_grounded.md) — structured slot fields (`tariff_cited`, `rate_change_pct_cited`) for citation-eligible facts; rubric backwards-compatible. **PR-A: foundation.** Schema, rubric, prompt template, 7 unit tests. No baselines change. PR-B will add slot-enforced JSON decoding (`lm-format-enforcer` or Outlines) and run the n=1000 A/B vs v1_constrained.
